...
if ( ! class_exists( 'WDS_Page_Builder_Areas' ) ) {
	...
	class WDS_Page_Builder_Areas {
      ...
      $area = $this->get_current_area();
			...
			$object_id = $layout[0]->ID;
			return $this->do_area( $area, $object_id );
		}
      ...
      $this->object_id = $post_id;
		...
		public function do_area( $area = '', $post_id = 0 ) {
			...
			// if post ID still zero, skip it. Maybe this archive or 404 page.
			$parts = ( 0 == $post_id ) ? false : $this->get_area( $area, $post_id );

			// If the area's parts are not set, use the default layout.
			if ( 0 == $this->object_id || ! $parts || 'none' == $parts[0] ) {
