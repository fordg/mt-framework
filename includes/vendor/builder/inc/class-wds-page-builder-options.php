...
class WDS_Page_Builder_Options {
	...
	public function add_options_page() {
		add_menu_page( __( 'Page Builder', 'wds-simple-page-builder' ), __( 'Page Builder', 'wds-simple-page-builder' ), 'edit_posts', 'edit.php?post_type=wds_pb_layouts', '', 'dashicons-list-view', 21 );
		$this->options_page = add_submenu_page( 'edit.php?post_type=wds_pb_layouts', $this->title, __( 'Global Layouts', 'wds-simple-page-builder' ), 'manage_options', $this->key, array( $this, 'admin_page_display' ) );
	...
	public function admin_page_display() {
		...
		wp_enqueue_script( 'wds-simple-page-builder-admin', $this->plugin->directory_url . '/assets/js/admin.js', array( 'jquery' ), PAGEBUILDER_VERSION, true );
		$tab = isset( $_GET['tab'] ) ? $_GET['tab'] : 'default-area-layouts';
		?>
  ...
  public function settings_notices( $object_id, $updated ) {
 		...
 		add_settings_error( $this->key . '-notices', '', __( 'Settings updated.', 'wds-simple-page-builder' ), 'updated' );
	...
	public function get_parts() {
		...
		if ( false === ( $parts = get_transient( 'spb_part_details' ) ) ) {
			...
			if (is_array($files) || is_object($files)) {
				foreach ( $files as $file ) {
					...
					if ( ! empty( $areas ) ) {
						$parts[ esc_attr( $slug ) ]['area'] = $areas;
					}
			...
			set_transient( 'spb_part_details', $parts, DAY_IN_SECONDS );
		}
		$this->parts = $parts;
		return $this->parts;
	}
