...
if ( ! class_exists( 'WDS_Page_Builder_Data' ) ) {
	...
	class WDS_Page_Builder_Data {
      ...
      if ( $this->plugin->areas->object_id ) {
        $post_id = $this->plugin->areas->object_id;
      } else {
        $post_id = !empty( $post_id ) ? $post_id : get_queried_object_id();
      }

      if ( ! $meta = get_post_meta( $post_id, '_wds_builder_layout_template', true ) ) {
        $meta = get_post_meta( $post_id, '_wds_builder_' . esc_attr( $area_key ) . 'template', true );
      }
