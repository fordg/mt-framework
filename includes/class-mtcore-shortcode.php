...
/**
 *  MT_Core_Shortcode - Minimal Themes Framework
 *
 * @author  Minimal Themes
 * @since 	1.0.0
 */
class MT_Core_Shortcode {
  ...
  protected $rel_path = 'includes/shortcodes';
    ...
    $this->version = MT_VERSION;
		if ( false !== ( $opts = get_option( 'mtcore_features' ) ) && in_array( 'shortcode', $opts ) ) {
	    $this->active = true;
	...
	public function register_mce_button() {
	  ...
	  if ( 'true' == get_user_option( 'rich_editing' ) ) {
			add_action( 'before_wp_tiny_mce', array( $this, 'generate_script' ), 9999 );
	...
	public function mce_external_plugins( $plugin_array ) {
		...
		if ( file_exists( trailingslashit($upload['basedir']) . 'mt-framework/shortcodes.js' ) ) {
			$plugin_array['mtcore'] = trailingslashit($upload['baseurl']) . 'mt-framework/shortcodes.js';
		}
	  return $plugin_array;
	}
    ...
    $menus = $this->menus;
    $cache = 'MT_'.md5( serialize( array($menus, $theme) ) );
    $files = trailingslashit($upload['basedir']) . 'mt-framework/shortcodes.js';
    ...
    if ( false === ( $mce = get_transient( $cache ) ) ) {
      ...
      $mce .= 'tinymce.create("tinymce.plugins.mtcore", {';
      $mce .= 'init : function(editor, url) {';
        ...
        $mce .= 'editor.addButton( "mtcore", {';
        $mce .= 'type: "menubutton",';
        $mce .= 'image: "'.MT_URI.'admin/js/shortcode-icon.png",';
        $mce .= 'tooltip: "Shortcode",';
        $mce .= 'menu: [';
      ...
      $mce .= '});'; // tinymce.create
      $mce .= 'tinymce.PluginManager.add( "mtcore", tinymce.plugins.mtcore );';
  ...
  private function generate_dialog( $menuitem ) {
	...
	public function register_shortcodes() {
		 ...
		 */
		$rel_path = apply_filters( 'mtcore_features_shortcode_path', $this->rel_path, self::$instance );

		/**
		 * Modify default main menu dropdown for shortcode
		 */
		$this->menus = apply_filters( 'mtcore_features_shortcode_menu',
		array(
			'content' => array(
				'text' => esc_attr__( 'Content', 'mtcore' ),
				'icon' => 'feedback',
				'menu' => array()
			),
			'element' => array(
				'text' => esc_attr__( 'Elements', 'mtcore' ),
				'icon' => 'networking',
				'menu' => array()
			),
			'boxed' 	=> array(
				'text' => esc_attr__( 'Boxed', 'mtcore' ),
				'icon' => 'editor-table',
				'menu' => array()
			),
			'media' 	=> array(
				'text' => esc_attr__( 'Media', 'mtcore' ),
				'icon' => 'admin-media',
				'menu' => array()
			),
			'other' 	=> array(
				'text' => esc_attr__( 'Others', 'mtcore' ),
				'icon' => 'portfolio',
				'menu' => array()
			),
		) );
		...
		if ( is_child_theme() ) {
			if ( is_dir( MT_CHILD . $rel_path ) ) {
				$this->load_shortcodes( MT_CHILD . $rel_path );
			}
		}

		//Check Theme Shortcode Overrides;
		if ( is_dir( MT_THEME . $rel_path ) ) {
			$this->load_shortcodes( MT_THEME . $rel_path );
		}
		 ...
		 * or remove all with:
		 * add_filter( 'mtcore_features_shortcode_directory', '__return_empty_array' );
		 *
		 */
		$dirs = apply_filters( 'mtcore_features_shortcode_directory',
			array( 'mtcore' => MT_INC . 'shortcodes' ),
			$this->shortcodes
		);
			...
			if ( is_array( $shortcode_files ) && count( $shortcode_files ) > 0 ) {
              ...
              $sc_data = get_file_data( $shortcode_file, array(
      					'name' => 'Name', 'desc' => 'Desc', 'close' => 'Ends', 'menu' => 'Menu'
							...
							$sc_data = apply_filters( 'mtcore_features_shortcode_data', $sc_data, $sc_base );
							...
							if ( !shortcode_exists( $sc_base ) && !empty($sc_data['name']) ) {
								...
								$sc_render = new MT_Core_Shortcode_Handler( $sc_base );
							}
						} // shortcode_exists
					} // if $is_shortcode
				} // endforeach
	...
	private function set_fields( $data ) {
		...
		$fields_head = array(
			'type'		=> 'container',
			'label'		=> esc_html__( 'Description', 'mtcore' ),
			'html'		=> $data['desc']
		);
		...
		foreach ($menu as $key => $value) {
          ...
          'enclose' => $data['close'],
					'fields'  => array(
						'title' => sprintf( __('%s Attributes','mtcore'), $data['name'] ),
						'body'  => $fields
					)
				);
			}
    ...
    if ( empty( $this->menus ) || $upload['error'] )
      return false;
		...
		$sc_real = file_put_contents( trailingslashit($upload['basedir']).'mt-framework/shortcodes.js', $sc_temp );
		return true;
	}
...
class MT_Core_Shortcode_Handler extends MT_Core_Shortcode {
