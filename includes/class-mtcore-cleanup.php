...
class MT_Core_Cleanup {
  ...
  /**
    ...
    $this->version = MT_VERSION;
		if ( false !== ( $opts = get_option( 'mtcore_features' ) ) && in_array( 'cleanup', $opts ) ) {
	    $this->active = true;
      add_action( 'init', array( 'MT_Core_Clean', 'wp_head' ) );
			add_action( 'init', array( 'MT_Core_Clean', 'generator' ) );
			add_action( 'admin_init', array( 'MT_Core_Clean', 'wp_admin' ) );
      add_action( 'plugins_loaded', array( 'MT_Core_Clean', 'core_updates' ) );
			add_action( 'registered_post_type', array( 'MT_Core_Clean', 'remove_revslider_metabox' ) );
  ...
  /**
    ...
    add_filter( 'style_loader_tag', array( 'MT_Core_Clean', 'style_loader_tag' ) );
    add_filter( 'script_loader_tag', array( 'MT_Core_Clean', 'script_loader_tag' ) );
    add_filter( 'script_loader_src', array( 'MT_Core_Clean', 'script_version' ) );
    add_filter( 'style_loader_src', array( 'MT_Core_Clean', 'script_version' ) );
    add_filter( 'body_class', array( 'MT_Core_Clean', 'body_class' ) );
	}
...
class MT_Core_Clean {
  ...
  static function wp_head() {
    ...
    add_filter('language_attributes', array('MT_Core_Clean', 'language_attributes'));
    ...
    if (!class_exists('WPSEO_Frontend')) {
      ...
      add_action('wp_head', array('MT_Core_Clean', 'rel_canonical'));
    }
  ...
  static function wp_admin() {
    add_action('wp_before_admin_bar_render', array( 'MT_Core_Clean', 'wp_logo' ), 0);
    add_filter('update_footer', array( 'MT_Core_Clean', 'update_footer' ), 998);
    add_filter('admin_footer_text', array( 'MT_Core_Clean', 'admin_footer_text' ), 998);
    add_action('admin_head', array( 'MT_Core_Clean', 'help_tabs' ), 99);
  ...
  static function core_updates() {
    add_action('admin_menu', array( 'MT_Core_Clean', 'update_menu' ), 999 );
  	add_action('init', array( 'MT_Core_Clean', 'wp_version_check' ), 1);
