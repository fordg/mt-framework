...
/**
 *  MT_Core_Widget - Minimal Themes Framework
 *
 * @author  Minimal Themes
 * @since 	1.0.0
 */
class MT_Core_Widget {
  ...
  public function __construct() {
    $this->version = MT_VERSION;
		if ( false !== ( $opts = get_option( 'mtcore_features' ) ) && in_array( 'widget', $opts ) ) {
	    $this->active = true;
	    $this->init();
		}
  }
  ...
  public function register_widgets() {
    ...
    $rel_path = apply_filters( 'mtcore_features_widget_path', $this->rel_path, self::$instance );
    ...
    if ( is_child_theme() ) {
      $source_dir[] = MT_CHILD . $rel_path;
    }
    $source_dir = apply_filters( 'mtcore_features_widget_source_dir', $source_dir, self::$instance );
    ...
    foreach ( $source_dir as $source ) {
      ...
      foreach ( $dir as $file ) {
        ...
        $widget_class = file_to_classname( $file );
   ...
   */
    ...
    if ( in_array( $screen->id, array( 'widgets', 'customize' ) ) )
		  include_once MT_PATH . 'admin/partials/mtcore-admin-icon.php';
	}
	...
	private function get_widget_key( $b, $i ){
		return 'MT_' . md5( serialize( array( $b, $i ) ) );
	}
 ...
 /**
  *  MT_Core_Widget_Factory - Minimal Themes Framework
  *
  * @author  Minimal Themes
  * @since 	1.0.0
  */
class MT_Core_Widget_Factory extends WP_Widget {
   ...
   * @return string
	...
	function update( $new_instance, $old_instance ) {
        ...
        break;
				...
				break;
				default :
					$instance[ $key ] = apply_filters( 'mtcore_widget_update_type_' . $setting[ 'type' ], $new_instance[ $key ], $key, $setting );
				break;
			}
		}
          ...
          $empty_option = ( isset($setting['empty']) && false !== $setting['empty'] ) ? '<option>-- none --</option>' : '';
              ...
              <?php echo $empty_option; ?>
					...
					<p>
						...
						<select class="widefat"<?php $this->set_attr( $atts ); ?>>
							...
							<option value="all" <?php selected( 'all', $value ); ?>>
								<?php printf( esc_attr__( 'All %s', 'mtcore' ), $tax_name ); ?>
							</option>
							<?php endif; ?>
				...
				case 'colorpicker' :
					...
					wp_enqueue_script(
						MT_SLUG.'-widget-color',
						MT_URI.'admin/js/admin-widget-color.js',
				...
				case 'iconpicker' :
					...
					wp_enqueue_style(
						MT_SLUG.'-widget-icon',
						MT_URI.'admin/css/admin-widget-icon.css',
						array(),
						MT_VERSION
					);
					wp_enqueue_script(
						MT_SLUG.'-widget-icon',
						MT_URI.'admin/js/admin-widget-icon.js',
						array( 'jquery' ),
						MT_VERSION,
						true
					);
					?>
						<p class="mt-iconpicker">
							...
							<span id="mt-icon-wrap" class="mt-icon-wrap">
								...
								<button type="button" class="mt-icon-select button button-small default"<?php echo !empty($value)?' style="display:none;"':''; ?>><?php esc_attr_e('Select Icon','mtcore'); ?></button>
							</span>
						</p>
					<?php
				break;
				case 'image' :
					wp_enqueue_media();
					wp_enqueue_script(
						MT_SLUG.'-widget-image',
						MT_URI.'admin/js/admin-widget-image.js',
						array( 'jquery' ),
						MT_VERSION,
						true
					);
				...
				?>
					...
					<p style="margin-top: 3px;">
						<a href="#" class="button-secondary <?php echo $field_id; ?>-add"><?php _e( 'Choose Image', 'mtcore' ); ?></a>
					</p>
					<p>
				...
				break;
				default :
					do_action( 'mtcore_widget_type_' . $setting[ 'type' ], $this, $key, $setting, $instance );
				break;
			} // switch type
		} // endforeach settings type

		echo '</div>'; //mtcore-widget-options
	}
